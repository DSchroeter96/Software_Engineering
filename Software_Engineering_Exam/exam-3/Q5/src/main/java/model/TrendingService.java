...
public class TrendingService {
    ...
    public CompletableFuture<List<GradedBook>> getTrending() {
        return this.database.getAllBooks().thenApply(gradedBooks ->
                gradedBooks.stream().sorted(Comparator.comparingInt(GradedBook::getGrade).reversed()).toList());
    }
    ...
    public CompletableFuture<List<GradedBook>> getTrending(int number) {
        return this.getTrending().thenApply(gradedBooks -> {
            if (number < 0 || number >= gradedBooks.size()) {
                throw new IllegalArgumentException("Specific number is not correct");
            }
            return gradedBooks.stream().limit(number).toList();
        });
    }
